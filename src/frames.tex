...
\begin{frame}
  ...
  \begin{itemize}
    \item insert to the main table
      \pause
      \begin{itemize}
        \item \textinline{11} $\mapsto$ \sqlinline{'{1, 3, 5, 3}'}
      \end{itemize}
      \pause
    \item insert to the secondary index
      ...
      \begin{itemize}
        ...
        \item write records \emph{entry} $\mapsto$ \emph{pk}:
          \pause
          \begin{itemize}
            \item \textinline{1} $\mapsto$ \textinline{11}
              \pause
            \item \textinline{3} $\mapsto$ \textinline{11}
              \pause
            \item \textinline{5} $\mapsto$ \textinline{11}
          \end{itemize}
      \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \sqlinline{-- previous: INSERT INTO arrs VALUES (11, '{1, 3, 5, 3}');}
  ...
  \begin{itemize}
    \item \textinline{1} $\mapsto$ \textinline{11}
    \item \textinline{1} $\mapsto$ \textinline{12}
    \item \textinline{2} $\mapsto$ \textinline{12}
    \item \textinline{3} $\mapsto$ \textinline{11}
    \item \textinline{3} $\mapsto$ \textinline{12}
    \item \textinline{3} $\mapsto$ \textinline{13}
    \item \textinline{5} $\mapsto$ \textinline{11}
    \item \textinline{5} $\mapsto$ \textinline{13}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item extract scan entries: \textinline{1}, \textinline{2}
      \pause
    ...
    \item next, a couple options:
      ...
      \begin{enumerate}
        ...
        \item do point index scans separately, deduplicate pkeys in postgres,
          then do table scans
        \item try IN bind: bind \textinline{a}$'$ IN \{\textinline{1},
          \textinline{2}\}, but make sure to deduplicate pkeys (e.g.\ the row
          with pkey \sqlinline{12})
          \pause
